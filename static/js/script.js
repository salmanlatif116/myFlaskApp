document
  ...
  .addEventListener("submit", async function (e) {
    ...
    if (url && aboutContact && includeDetail) {
      resultDiv.innerHTML =
        "Scraping emails from both sources, You'll get email once scraping done.";
      ...
      try {
        ...
        resultDiv.innerHTML =
          combinedEmails.length > 0
            ? `<h3>Emails Found:</h3><ul>${combinedEmails
                .map(
                  (email) => `<li style="list-style-type: none;">${email}</li>`
                )
                .join("")}</ul>`
      ...
      resultDiv.innerHTML =
        "Scraping about & contact emails, You'll get email once scraping done.";

      try {
        const response = await fetch(
          "/api/v1/url/scrape-about-contact-and-send-emails/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        resultDiv.innerHTML =
          data.Emails && data.Emails.length > 0
            ? `<h3>Emails Found:</h3><ul>${data.Emails.map(
                (email) => `<li style="list-style-type: none;">${email}</li>`
              ).join("")}</ul>`
      ...
      resultDiv.innerHTML =
        "Scraping detail emails, You'll get email once scraping done.";

      try {
        const response = await fetch(
          "/api/v1/url/scrape-all-and-send-emails/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    } else if (query && aboutContact && includeDetail) {
      resultDiv.innerHTML =
        "Scraping emails from both sources, You'll get email once scraping done.";
      ...
      try {
        const [aboutContactResponse, includeDetailResponse] = await Promise.all(
          [
            fetch("/api/v1/scrape/google-map-about-contact/", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
            ...
            fetch("/api/v1/scrape/google-map-all-urls", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
    ...
    } else if (query && aboutContact) {
      // Existing query fetch logic
      resultDiv.innerHTML =
        "Scraping URLs, please wait. You will get email once scraping is done.";

      try {
        const response = await fetch(
          "/api/v1/scrape/google-map-about-contact/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    } else if (query && includeDetail) {
      // Existing query fetch logic
      resultDiv.innerHTML =
        "Scraping URLs, please wait. You will get email once scraping is done.";

      try {
        const response = await fetch("/api/v1/scrape/google-map-all-urls", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      resultDiv.innerHTML =
        "<p>Please enter either a URL or a query or select any checkbox.</p>";
    }
  });
